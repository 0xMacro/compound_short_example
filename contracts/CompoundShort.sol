...
contract CompoundShort {
    ...
    IUniswapV2Pair public uniPair =
        IUniswapV2Pair(0xEBFb684dD2b01E698ca6c14F10e4f289934a54D6);
    ...
    function supply(uint256 mintAmount) external payable {
        ...
        console.log(
            "USDC balance of CompoundShort.sol: %s",
            tokenCollateral.balanceOf(address(this)) / 1e6
        );
    ...
    function short(uint256 borrowAmount) external {
        ...
        console.log(
            "UNI balance of CompoundShort.sol: %s",
            tokenBorrow.balanceOf(address(this)) / 1e18
        );
        console.log(
            "USDC balance of CompoundShort.sol: %s",
            tokenCollateral.balanceOf(address(this)) / 1e6
        ...
        // sell UNI for USDC
        uint256 uniBalance = tokenBorrow.balanceOf(address(this));
    ...
    function lowerUNIPriceOnUniswap(uint256 uniAmountToSell) external {
        console.log(
            "------------------- beginning of lowerUNIPriceOnUniswap -------------------"
        );
    ...
    function repayBorrow() external {
        ...
        tokenCollateral.approve(
            address(UNI),
            tokenCollateral.balanceOf(address(this))
        );
        ...
        uint256 cTokenBalance = cTokenCollateral.balanceOf(address(this));
        console.log("cTokenBalance: %s", cTokenBalance);

        uint256 balanceOfUnderlying = cTokenCollateral.balanceOfUnderlying(
            address(this)
        );
        ...
        uint256 totalUSDCBalance = tokenCollateral.balanceOf(address(this)) +
            balanceOfUnderlying;
        console.log(
            "USDC balance of CompoundShort.sol: %s",
            totalUSDCBalance / 1e6
        );
        console.log(
            "UNI balance of CompoundShort.sol: %s",
            tokenBorrow.balanceOf(address(this)) / 1e18
